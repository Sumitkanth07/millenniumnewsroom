    ...
    <div class="article-list">
        ...
        @empty
            <div class="card empty-state" style="margin-bottom: 40px; text-align: center; padding: 40px 20px;">
                ...
                <p style="color: #8e7d61; font-size: 16px;">New articles in {{ $category->name }} will appear here soon.</p>
            </div>
            
            @if(!empty($fallbackData))
                @php
                    $sections = [
                        'Latest News' => $fallbackData['latestNews'] ?? collect(),
                        'Trending Stories' => $fallbackData['trendingStories'] ?? collect(),
                        'Editor\'s Picks' => $fallbackData['editorPicks'] ?? collect(),
                        'Recent Articles' => $fallbackData['recentArticles'] ?? collect(),
                    ];
                @endphp

                @foreach($sections as $sectionTitle => $sectionPosts)
                    @if($sectionPosts->isNotEmpty())
                        <div class="fallback-section" style="margin-bottom: 40px; width: 100%;">
                            <h2 style="font-family: Georgia, serif; font-size: 22px; border-bottom: 2px solid #c79a2b; padding-bottom: 8px; margin-bottom: 20px; color: inherit;">{{ $sectionTitle }}</h2>
                            <div class="responsive-grid-auto" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 20px;">
                                @foreach($sectionPosts as $fallbackPost)
                                    @php
                                    ...
                                    <article class="card" style="display: flex; flex-direction: column; height: 100%; padding: 15px;">
                                        @if($fallbackImage)
                                            <div style="aspect-ratio: 16/9; overflow: hidden; border-radius: 6px; margin-bottom: 12px; background: #120f0b;">
                                                <img src="{{ $fallbackPost->getThumbnailUrl() }}" alt="{{ $fallbackPost->featured_image_alt ?: $fallbackPost->title }}" width="400" height="225" style="width: 100%; height: 100%; object-fit: cover;" loading="lazy">
                                            </div>
                                        @endif
                                        <div style="display: flex; flex-direction: column; flex: 1;">
                                            ...
                                            <h3 style="font-family: Georgia, serif; font-size: 16px; margin: 6px 0; line-height: 1.3;"><a href="{{ $fallbackPost->publicUrl() }}" style="color: inherit; text-decoration: none;">{{ $fallbackPost->title }}</a></h3>
                                            <p style="font-size: 13px; color: #8e7d61; line-height: 1.4; margin-top: auto;">{{ Str::limit($fallbackPost->excerpt, 80) }}</p>
                                        </div>
                                    </article>
                                @endforeach
                            </div>
